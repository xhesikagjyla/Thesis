...
class MessengerController extends Controller
{
    ...
    function fetchMessages(Request $request)
    {
        ...
        if (count($messages) < 1) {
            $response['messages'] = "<div class='d-flex justify-content-center no_messages align-items-center h-100'>
            <p> Say 'hi' to start a conversation</p></div>";
    ...
    function fetchContacts(Request $request){
        ...
        if (count($users) > 0) {
            ...
            $contacts="<p class='text-center no_contacts'>Your contact list is empty!</p>";
        }
        return response()->json([
            'contacts'=>$contacts,
